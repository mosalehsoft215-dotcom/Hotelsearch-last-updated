...
async def main() -> int:
    ...
    try:
        ...
        for o in options[:5]:
            ...
            print(f"        {o.roomDescription} / {o.boardText}  "
                  f"{getattr(o.price, 'totalPrice', None)} {getattr(o.price, 'currency', '')}  "
                  f"refundable={getattr(policy, 'refundable', None)}")
            # The option id is what refresh_hotel_price takes as optionRefId.
            print(f"          optionId={(o.optionId or '')[:60]}...")
    except Exception as exc:
