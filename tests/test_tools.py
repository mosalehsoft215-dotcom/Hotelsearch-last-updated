...
async def test_static_data_core_selection(fake_hasura):
    fake_hasura.responses["getSearchHotelStaticData"] = {
        ...
        "addressLines": ["1 Main St", "Metroville 23425"], "medias": ["http://img/1.jpg"],
    ...
    assert out.addressLines == ["1 Main St", "Metroville 23425"]
    assert out.medias == ["http://img/1.jpg"]
...
@pytest.mark.asyncio
async def test_static_data_selects_medias_and_facilities_without_a_subselection(fake_hasura):
    """HotelObj.medias and HotelObj.facilities are [String]. Asking for a
    subselection on either is rejected by the backend, so the query must not."""
    ...
    query = fake_hasura.calls[0]["query"]
    assert "medias {" not in query and "facilities {" not in query
    assert "facilities" in query


@pytest.mark.asyncio
async def test_static_data_extras_opt_in(fake_hasura):
    fake_hasura.responses["getSearchHotelStaticData"] = {"hotelCode": "H1"}
    await srv.get_hotel_static_data(hotelCode="H1", extras=["descriptions"])
    assert "descriptions {" in fake_hasura.calls[0]["query"]
    with pytest.raises(ValueError):
        await srv.get_hotel_static_data(hotelCode="H1", extras=["nope"])


@pytest.mark.asyncio
async def test_hotel_options_builds_criteria_and_flattens_room_search(fake_hasura):
    """getHotelFullOptions returns options: [RoomSearch], the same rooms /
    roomsOptions pair the availability call returns — not a flat option list."""
    fake_hasura.responses["getHotelFullOptions"] = {
        "hotelId": "H1", "uuid": "U9",
        "options": [
            {"rooms": [{"code": "R1", "description": "King Room"}],
             "roomsOptions": [
                 {"id": "OPT-1", "boardText": "Room Only", "status": "OK",
                  "price": {"totalPrice": 220, "currency": "USD"},
                  "cancelPolicy": {"refundable": True, "description": "free until 48h"}},
                 {"id": "OPT-2", "boardText": "Breakfast", "status": "OK",
                  "price": {"totalPrice": 500, "currency": "USD"},
    ...
    assert out.uuid == "U9"
    # The option id is what refresh_hotel_price takes as optionRefId.
    assert [o.optionId for o in out.options] == ["OPT-1"]
    assert out.options[0].roomDescription == "King Room"
    v = fake_hasura.calls[0]["variables"]
