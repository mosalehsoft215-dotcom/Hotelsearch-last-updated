...
_Q_STATIC_CORE = """hotelCode hotelId hotelName chainId rating cityName country countryCode
    street postalCode geoLocation checkInTime checkOutTime giataCityId lastUpdate
    medias { url type code order } phones { tech value }"""

# descriptions/facilities inner types are inferred (HotelDescription / Amenity);
# they are opt-in so the default selection stays on confirmed ground.
_Q_STATIC_EXTRAS = {
    "descriptions": "descriptions { lang lastUpdate sections { title para type } }",
    "facilities": "facilities { code type value texts { language text } }",
}
...
query getHotelFullOptions($applyMarkup: Boolean!, $criteria: HotelCriteriaSearchInput!,
  ...
  getHotelFullOptions(applyMarkup: $applyMarkup, criteria: $criteria,
    ...
    hotelId uuid
    options {
      optionRefId status remarks
      price { totalPrice net gross currency }
      searchPrice { net gross currency }
      cancelPolicy { refundable description
        cancelPenalties { hoursBefore penaltyType value currency deadline } }
    }
  }
}
""".strip()
...
async def get_hotel_options(
    ...
    """Priced room options for one hotel, each with an optionRefId to reprice and
    book. Not tied to a search session, so use it when the search uuid has
    expired. Wraps `getHotelFullOptions`.
    """
    currency = currency or _settings.default_currency
    nationality = nationality or _settings.default_nationality
    require_common_args(organization_id=organizationId, currency=currency, nationality=nationality)
    ...
    with log_tool_call("get_hotel_options",
        ...
        result = HotelOptionsResult.model_validate(data)
        result.options = filter_options(result.options or [], refundable_only=refundableOnly,
                                        min_price=minPrice, max_price=maxPrice)
        return result
